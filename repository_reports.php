...
if(isset($_GET['search'])){
  ...
  $sql = "SELECT rr.name AS name, rr.code AS code, rr.date AS date, rr.number AS number, rr.old_number AS old_number,rr.status AS status,rr.burrow_number AS burrow_number, p.name AS user_name FROM repository_reports rr JOIN personnel p ON rr.user_id = p.code WHERE rr.name LIKE '%$search_term%' OR rr.number LIKE '%$search_term%' OR p.name LIKE '%$search_term%'  ORDER BY rr.id DESC;";
  $stmt = $pdo->prepare($sql);
  $stmt->execute();
  $reports = $stmt->fetchAll(PDO::FETCH_ASSOC);
}else{
  $query = "SELECT rr.name AS name, rr.code AS code, rr.date AS date, rr.number AS number, rr.old_number AS old_number,rr.status AS status,rr.burrow_number AS burrow_number, p.name AS user_name FROM repository_reports rr JOIN personnel p ON rr.user_id = p.code ORDER BY rr.id DESC Limit 10 OFFSET $offset;";
   ...
   <body>
     <div class="container-fluid h-auto">
       <div class="row h-100">
        ...
        <div class="col-lg-11 col p-5 mt-lg-0 mt-5 animation-pop">
            <div class="row flex-row d-flex p-0 m-0 align-items-center mb-3 justify-contet-between">
              <p class="display-1 col-lg-6 col align-content-center">گزارشات انبار</p>
              <div class="col d-flex align-self-center justify-content-end p-0">
                <a href="repository.php"><button class="btn-type">بازگشت به انبار</button></a>
              </div>
            </div>
            <div class="row d-flex flex-row p-0 m-0 justify-content-between mt-2 mt-lg-0">
              <div class="col p-0 align-content-center">
                <ul class="nav nav-pills mb-3 pe-0" id="pills-tab" role="tablist">
                  <li class="nav-item" role="presentation">
                    <button class="nav-link g60 active" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-home" type="button" role="tab" aria-controls="pills-home" aria-selected="true">همه گزارشات</button>
                  </li>
                </ul>
              </div>
              <div class="col-lg-5 col flex-column flex-lg-row d-flex p-0 ps-0 align-content-center">
            ...
            <div class="col flex-column d-lg-none" id="responsive-table">
              ...
              <div class="col flex-column base mt-3">
                ...
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  <span class="flex-start fb-16 g60">نام کاربر</span>
