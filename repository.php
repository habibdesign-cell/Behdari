...
// Permission Check -----------------
if ($_SESSION['repository']==0) {
    header("Location: 404.php");
    exit();
}
//end --------------------------------
$current_date = translatenowDate();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $name = $_POST['name'];
    $code = $_POST['code'];
    $org_number = $_POST['organization_number'];
    $real_number = $_POST['real_number'];
    $burrow_number = $_POST['burrow_number'];
    $repository = $_POST['repository'];
    $sql = "SELECT * FROM repository WHERE code=$code";
    $result = $conn->query($sql);
    $good = $result->fetch_assoc();
    $old_number = $good['real_number'];

    if (empty($good)) {
        // Add new drug
        $sql = "INSERT INTO repository (name, code, organization_number, real_number, burrow_number, repository)
                VALUES ('$name', '$code', '$org_number', '$real_number', '$burrow_number', '$repository')";
                $status_r= 'active';
      ...
      $status_r='edit';
        ...
        $sql = "UPDATE repository SET
                ...
                burrow_number='$burrow_number',
                repository='$repository'
                WHERE code='$code'";
        $success_message = "Drug updated successfully!";
    }
    if ($conn->query($sql) === TRUE) {
      $query = "INSERT INTO repository_reports (code,name,date, number,old_number, burrow_number,user_id, status)
              VALUES ('$code','$name','$current_date', '$real_number','$old_number','$burrow_number','$user_id', '$status_r')";
              $conn->query($query);
...
if (isset($_GET['delete_id'])) {
    ...
    $sql2 = "SELECT * FROM repository WHERE code= :id";
    $stmt = $pdo->prepare($sql2);
    $stmt->execute(['id'=>$id]);
    $good = $stmt->fetch(PDO::FETCH_ASSOC);
    $code = $good['code'];
    $real_number = $good['real_number'];
    $burrow_number = $good['burrow_number'];
    if ($conn->query($sql) === TRUE) {
      $status_r = 'leave';
        $query = "INSERT INTO repository_reports (code,name,date, number,old_number, burrow_number,user_id, status)
                VALUES ('$code','$name','$current_date', '$real_number','$old_number','$burrow_number','$user_id', '$status_r')";
...
if(isset($_GET['search'])){
  ...
  $sql = "SELECT * FROM repository WHERE name LIKE '%$search_term%' OR code LIKE '%$search_term%' ORDER BY id DESC";
  $stmt = $pdo->prepare($sql);
  $stmt->execute();
   ...
   <body>
     <div class="container-fluid h-auto">
       <div class="row h-100">
               ...
               <?php if($_SESSION['repository']) : ?>
                 <a href="repository.php">
                   <div class="fb-14 d-flex align-items-center primaryc px-2 py-3 strokem">
                     <img class="mx-2" src="img/icon-box-active.png" width="16px" height="16px" alt="">
                     انبار بهداری
                   </div>
                 </a>
               <?php endif; ?>
        ...
        <div class="col col-lg-11 p-5 mt-lg-0 mt-5 animation-pop">
            <div class="row flex-row d-flex p-0 m-0 align-items-center mb-3 justify-contet-between">
              <p class="display-1 col-lg-6 col align-content-center">انبار بهداری</p>
              <div class="col-6 d-flex justify-content-end p-0">
                <a class="w-lg-25 me-3 ms-3" href="repository_reports.php"><button class="btn-type">گزارشات</button></a>
                <button id="button-form" class="btn-type">بستن و نمایش فرم</button>
              </div>
            </div>
            <div class="row mb-3">
              <form method="POST" class=" animation-fade" action="repository.php<?php if(isset($_GET['id'])){echo '?id='.htmlspecialchars($good['code']);} ?>" id="form" style="<?php if(isset($_GET['id'])){echo "display:block;";}else{echo "display:none;";} ?>">
                <div class="d-flex flex-lg-row flex-column flex-row">
                  <div class="ms-lg-3 col mt-lg-0 mt-3">
                    <label for="personnel_code">نام جنس</label>
                    <input value="<?php if(isset($_GET['id'])){echo htmlspecialchars($good['name']);} ?>" name="name" type="text" class="form-control-simple mt-lg-0 mt-3 ms-lg-3" placeholder="نام جنس" required>
                  </div>
                  <div class="ms-lg-3 col mt-lg-0 mt-3">
                    <label for="personnel_code">کد جنس</label>
                    <input value="<?php if(isset($_GET['id'])){echo htmlspecialchars($good['code']);} ?>" name="code" type="text" class="form-control-simple mt-lg-0 mt-3 ms-lg-3" placeholder="کد جنس" required>
                  </div>
                  <div class="col mt-lg-0 mt-3">
                    <label for="personnel_code">انتخاب انبار</label>
                    <select class="input-group-prepend w-100 form-control h-75 fb-14 g60 dropdown-toggle" name="repository"  value="<?php if(isset($_GET['id'])){echo htmlspecialchars($good['repository']);} ?>" required>
                      <option class="fb-14" value="1" <?php if(isset($_GET['id'])){if($good['repository']==1){echo "selected";}} ?>>انبار سررشته داری</option>
                      <option class="fb-14" value="2" <?php if(isset($_GET['id'])){if($good['repository']==2){echo "selected";}} ?>>انبار تجهیزات پزشکی</option>
                    </select>
                  </div>
                </div>
                <div class="d-flex flex-lg-row flex-column mt-lg-3 flex-row">
                  <div class="ms-lg-3 col mt-lg-0 mt-3">
                    <label for="personnel_code">سازمانی</label>
                    <input value="<?php if(isset($_GET['id'])){echo htmlspecialchars($good['organization_number']);} ?>" name="organization_number" type="text" class="form-control-simple mt-lg-0 mt-3 ms-lg-3" placeholder="سازمانی" required>
                  </div>
                  <div class="ms-lg-3 col mt-lg-0 mt-3">
                    <label for="personnel_code">موجودی</label>
                    <input value="<?php if(isset($_GET['id'])){echo htmlspecialchars($good['real_number']);} ?>" name="real_number" type="text" class="form-control-simple mt-lg-0 mt-3 ms-lg-3" placeholder="موجودی" required>
                  </div>
                  <div class="ms-lg-3 col mt-lg-0 mt-3">
                    <label for="personnel_code">قرض</label>
                    <input value="<?php if(isset($_GET['id'])){echo htmlspecialchars($good['burrow_number']);} ?>" name="burrow_number" type="text" class="form-control-simple mt-lg-0 mt-3 ms-lg-3" placeholder="قرض" required>
                  </div>
              ...
              <div class="col p-0 align-content-center">
                <ul class="nav nav-pills mb-3 pe-0" id="pills-tab" role="tablist">
                  <li class="nav-item" role="presentation">
                    <button class="nav-link g60 active" id="pills-all-tab" data-bs-toggle="pill" data-bs-target="#pills-all" type="button" role="tab" aria-controls="pills-all" aria-selected="true">همه انبارها</button>
                  </li>
                  <li class="nav-item" role="presentation">
                    <button class="nav-link g60" id="pills-1-tab" data-bs-toggle="pill" data-bs-target="#pills-1" type="button" role="tab" aria-controls="pills-1" aria-selected="false">سررشته داری</button>
                  </li>
                  <li class="nav-item" role="presentation">
                    <button class="nav-link g60" id="pills-2-tab" data-bs-toggle="pill" data-bs-target="#pills-2" type="button" role="tab" aria-controls="pills-2" aria-selected="false">تجهیزات پزشکی</button>
                  </li>
                </ul>
              </div>
              <div class="col-lg-5 col flex-row d-flex p-0 ps-0 align-content-center">
                <a class="col  mb-3 me-3 " href="export_csv.php?name=repository">
                  <button class="w-100 py-3 py-lg-0 h-100 btn-small" type="button" name="button">چاپ لیست تجهیزات</button>
                </a>
            ...
            <div class="col flex-column d-lg-none" id="responsive-table">
              <?php if (count($goods) > 0): ?>
                <?php foreach ($goods as $row): ?>
              <div class="col flex-column base mt-3 responsive-row" data-status='<?php echo $row["repository"]; ?>'>
                    ...
                    <div class="action-menu">
                      ...
                      <path d="M3.3335 10C2.80306 10 2.29436 9.78929 1.91928 9.41421C1.54421 9.03914 1.3335 8.53043 1.3335 8C1.3335 7.46957 1.54421 6.96086 1.91928 6.58579C2.29436 6.21071 2.80306 6 3.3335 6C3.86393 6 4.37264 6.21071 4.74771 6.58579C5.12278 6.96086 5.3335 7.46957 5.3335 8C5.3335 8.53043 5.12278 9.03914 4.74771 9.41421C4.37264 9.78929 3.86393 10 3.3335 10ZM12.6668 10C12.1364 10 11.6277 9.78929 11.2526 9.41421C10.8775 9.03914 10.6668 8.53043 10.6668 8C10.6668 7.46957 10.8775 6.96086 11.2526 6.58579C11.6277 6.21071 12.1364 6 12.6668 6C13.1973 6 13.706 6.21071 14.081 6.58579C14.4561 6.96086 14.6668 7.46957 14.6668 8C14.6668 8.53043 14.4561 9.03914 14.081 9.41421C13.706 9.78929 13.1973 10 12.6668 10ZM8.00016 10C7.46973 10 6.96102 9.78929 6.58595 9.41421C6.21088 9.03914 6.00016 8.53043 6.00016 8C6.00016 7.46957 6.21088 6.96086 6.58595 6.58579C6.96102 6.21071 7.46973 6 8.00016 6C8.5306 6 9.0393 6.21071 9.41438 6.58579C9.78945 6.96086 10.0002 7.46957 10.0002 8C10.0002 8.53043 9.78945 9.03914 9.41438 9.41421C9.0393 9.78929 8.5306 10 8.00016 10Z" fill="#697077"/>
                        ...
                        <ul class="menu-options">
                            <li><a class="fb-14 dropdown-section" href="repository.php?id=<?php echo $row['code']; ?>">ویرایش</a></li>
                            <li><a class="fb-14 dropdown-del" href="repository.php?delete_id=<?php echo $row['code']; ?>">حذف</a></li>
                        </ul>
                    </div>
                  </span>
                </div>
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  <span class="flex-start fb-16 g60">	نام جنس</span>
                  <span class="flex-end fb-16 g100"><?php echo htmlspecialchars($row['name']); ?></span>
                </div>
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  <span class="flex-start fb-16 g60">	کد جنس</span>
                  <span class="flex-end fb-16 g100"><?php echo htmlspecialchars($row['code']); ?></span>
                </div>
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  <span class="flex-start fb-16 g60">	سازمانی</span>
                  <span class="flex-end fb-16 g100"><?php echo htmlspecialchars($row['organization_number']); ?></span>
                </div>
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  <span class="flex-start fb-16 g60">موجودی</span>
                  <span class="flex-end fb-16 g100"><?php echo htmlspecialchars($row['real_number']); ?></span>
                </div>
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  <span class="flex-start fb-16 g60">قرض</span>
                  <span class="flex-end fb-16 g100"><?php echo htmlspecialchars($row['burrow_number']); ?></span>
                </div>
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  <span class="flex-start fb-16 g60">انبار</span>
                  <span class="flex-end fb-16 g100"><?php echo translateRepository(htmlspecialchars($row['repository'])); ?></span>
                </div>
              </div>
            <?php endforeach; ?>
              <?php else: ?>
            ...
            <table class="table base d-non" id="repository-table">
              <thead class="thead-light">
                <tr style="LINE-HEIGHT:32px">
                  <th scope="col"></th>
                  <th scope="col">نام جنس</th>
                  <th scope="col">کد جنس</th>
                  <th scope="col">سازمانی</th>
              ...
              <tbody>
                <?php if (count($goods) > 0): ?>
                  <?php foreach ($goods as $row):
                    echo "<tr class='fb-16 g100' data-status='{$row['repository']}'>"
                    ?>
                      <th>
                       <input class="form-check-input" type="checkbox" value="" id="flexCheckDefault">
                      </th>
                      <td class="number"><?php echo htmlspecialchars($row['name']); ?></td>
                      <td class=""><?php echo htmlspecialchars($row['code']); ?></td>
                      <td class=""><?php echo htmlspecialchars($row['organization_number']); ?></td>
                      <td class=""><?php echo htmlspecialchars($row['real_number']); ?></td>
                      <td class=""><?php echo htmlspecialchars($row['burrow_number']); ?></td>
                      <td class=""><?php echo translateRepository(htmlspecialchars($row['repository'])); ?></td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const tableRows = document.querySelectorAll('#repository-table tbody tr');
