...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM drugs WHERE code=$code";
    $result = $conn->query($sql);
    $drug = $result->fetch_assoc();
    $old_number = $drug['real_number'];
    if (empty($drug)) {
        // Add new drug
        $sql = "INSERT INTO drugs (name, code, organization_number, real_number)
                VALUES ('$name', '$code', '$org_number', '$real_number')";
                $status_r= 'active';
    ...
    if ($conn->query($sql) === TRUE) {
      $query = "INSERT INTO drugs_reports (code,name,date, number,user_id, status, old_number)
              VALUES ('$code','$name','$current_date', '$real_number','$user_code', '$status_r', '$old_number')";
              $conn->query($query);
...
if (isset($_GET['delete_id'])) {
    ...
    $sql = "DELETE FROM drugs WHERE code='$id'";

    $sql2 = "SELECT * FROM drugs WHERE code='$id'";
    $result = $conn->query($sql2);
    $drug = $result->fetch_assoc();
    $code = $drug['code'];
    $real_number = $drug['real_number'];
    $drug_name = $drug['name'];
    if ($conn->query($sql) === TRUE) {
      $status_r = 'leave';
        $query = "INSERT INTO drugs_reports (code,name,date, number,user_id, status, old_number)
                VALUES ('$code','$drug_name','$current_date', '$real_number','$user_code', '$status_r', '$old_number')";
                $conn->query($query);
        header("Location: pharmacy.php");
    } else {
        echo "<p>Error: " . $sql . "<br>" . $conn->error . "</p>";
...
if(isset($_GET['search'])){
  ...
  $sql = "SELECT * FROM drugs WHERE name LIKE '%$search_term%'";
  $stmt = $pdo->prepare($sql);
  $stmt->execute();
   ...
   <body>
     <div class="container-fluid h-auto">
       <div class="row h-100">
        ...
        <div class="col p-lg-5 p-3 mt-lg-0 mt-5 pt-5 pt-lg-0 animation-pop">
            <div class="row flex-row d-flex p-0 m-0 align-items-center mb-0 justify-contet-between">
              <p class="display-1 col-lg-6 col align-content-center">داروخانه</p>
              <div class="col-6 d-flex align-self-center justify-content-end p-0">
                <a class="w-lg-25 me-3 ms-3" href="pharmacy_reports.php"><button class="btn-type">گزارشات</button></a>
                <button id="button-form" class="btn-type">بستن و نمایش فرم</button>
              </div>
            </div>
            <div class="row mb-3">
              <form method="POST" class="animation-fade" action="pharmacy.php<?php if(isset($_GET['id'])){echo '?id='.htmlspecialchars($drug['code']);} ?>" id="form" style="<?php if(isset($_GET['id'])){echo "display:block;";}else{echo "display:none;";} ?>">
                <div class="d-flex flex-lg-row flex-column flex-row ">
                  <div class="ms-lg-5 col">
                    <label for="name">نام دارو</label>
                    <input value="<?php if(isset($_GET['id'])){echo htmlspecialchars($drug['name']);} ?>" name="name" type="text" class="form-control-simple mt-lg-0 mt-3 ms-lg-3" placeholder="وارد کنید ..." required >
                  </div>
                  <div class="ms-lg-5 col">
                    <label for="code">کد دارو</label>
                    <input value="<?php if(isset($_GET['id'])){echo htmlspecialchars($drug['code']);} ?>" name="code" type="text" class="form-control-simple mt-lg-0 mt-3 ms-lg-3" placeholder="قابل تغییر نمی باشد" required >
                  </div>
                  <div class="ms-lg-5 col">
                    <label for="organization_number">سازمانی</label>
                    <input value="<?php if(isset($_GET['id'])){echo htmlspecialchars($drug['organization_number']);} ?>" name="organization_number" type="text" class="form-control-simple mt-lg-0 mt-3 ms-lg-3" placeholder="تعدادی که طبق آمار باید موجود باشد" required>
                  </div>
                  <div class="col ms-lg-5">
                    <label for="real_number">موجودی</label>
                    <input value="<?php if(isset($_GET['id'])){echo htmlspecialchars($drug['real_number']);} ?>" name="real_number" type="text" class="form-control-simple mt-lg-0 mt-3 ms-lg-3" placeholder="تعدادی که هم اکنون موجود است" required>
                  </div>
                  <button class="me-3 col h-50 align-self-end w-100 mt-lg-0 mt-3">تایید اطلاعات</button>
                </div>
              </form>
            </div>
            <div class="row d-flex flex-row p-0 m-0 justify-content-between mt-3 mt-lg-0">
              <div class="col-2 col p-0 align-content-center d-non">
              ...
              <div class="col-lg-5 col flex-row d-flex p-0 ps-0 align-content-center">
                <a class="col  mb-3 me-3 " href="export_csv.php?name=drugs">
                  <button class="w-100 py-3 py-lg-0 h-100 btn-small" type="button" name="button">چاپ لیست داروها</button>
                </a>
            ...
            <div class="col d-flex flex-column d-lg-none">
              ...
              <div class="col d-flex flex-column base mt-3">
                ...
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  <span class="flex-start fb-16 g60">نام دارو</span>
                  <span class="flex-end fb-16 g100"><?php echo htmlspecialchars($row['name']); ?></span>
                </div>
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  <span class="flex-start fb-16 g60">کد دارو</span>
                ...
                <div class="px-3 py-2 d-flex flex-row justify-content-between bottom-line">
                  ...
                  <span class="flex-end fb-16 <?php if($row['real_number']>0){echo 'sbadge' ;}else{echo 'abadge' ;} ?> fb-14"><?php if($row['real_number']>0){echo 'موجود' ;}else{echo 'ناموجود' ;} ?></span>
                </div>
              </div>
            <?php endforeach; ?>
              <?php else: ?>
                ...
                <?php if (count($drugs) > 0): ?>
                  <?php foreach ($drugs as $row): ?>
                    <tr class="fb-16 g100" >
                      ...
                      <td class=""><span class="<?php if($row['real_number']>0){echo 'sbadge' ;}else{echo 'abadge' ;} ?> fb-14"><?php if($row['real_number']>0){echo 'موجود' ;}else{echo 'ناموجود' ;} ?></span></td>
